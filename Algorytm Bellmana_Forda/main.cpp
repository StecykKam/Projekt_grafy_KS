...
int* Bellman_Forda_algorithm(Graph& graph,int number_of_nodes)  // wlasciwy algorytm
{
   ...
   // int table_of_values_of_nodes[number_of_nodes]; // global table for Bellman Ford values paths
    ...
    {   
        table_of_values_of_nodes[i] =  std::numeric_limits<int>::max();
    }
    ...
    {   
        ...
        if(i == number_of_nodes - 1 && counter_of_changes > 0)
        {
    ...
    {
        cout << "Shortest path to node " << i << " is equal " << table_of_values_of_nodes[i] << endl;
    }
...
{   
    ...
    string name_of_file_to_read = "proba.txt";  /// <------------ tutaj wpisujemy nazwe pliku txt , wazne aby node'y byly numerowane od 0...n
